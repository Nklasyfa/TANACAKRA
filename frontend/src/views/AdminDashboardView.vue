...
// Export functions
const exportLogsCsv = () => {
  downloadCsv('tanacakra_audit_logs.csv', activityLogs.map(l => ({
    Waktu: l.time,
    Aktivitas: l.title,
    Operator: l.operator,
    Status: l.status
...
// Activity logs static fallback combined with dynamic info
const activityLogs = [
  {
    time: '24 Okt 07:30',
    title: 'Sinkronisasi data telemetri 4 node sensor tanah',
    operator: 'Sistem Otomatis (Node Cangkringan-1)',
    status: 'Berhasil',
    statusClass: 'bg-[#d5e9c3] text-[#243319] border border-[#bacda8]'
  },
  {
    time: '24 Okt 06:15',
    title: 'Pembaruan input observasi fisik lahan Blok B',
    operator: 'Pak Supardi (Poktan Cangkringan)',
    status: 'Tercatat',
    statusClass: 'bg-[#e8ded7] text-[#4c4641] border border-[#cfc5be]'
  },
  {
    time: '23 Okt 23:00',
    title: 'Eksekusi model prediksi panen & harga lelang',
    operator: 'Pipeline AI Tanacakra v2.4',
    status: 'Selesai',
    statusClass: 'bg-[#dee5d8] text-[#171d16] border border-[#c2c9bc]'
  }
]
...
const loadData = async () => {
  ...
  const [weatherRes, lahans, trends, users] = await Promise.all([
          ...
          <div class="flex flex-col gap-3">
            ...
            <div class="rounded-xl p-4 bg-[#FFF5F2] border-l-4 border-l-[#C84C32] border border-[#E5E0D8]/60 shadow-xs">
              ...
              <div class="flex justify-end pt-2">
                <button class="px-3 py-1 rounded-lg bg-white text-[#C84C32] border border-[#E5E0D8] text-xs font-bold hover:bg-[#FFF5F2] transition-colors shadow-2xs">
                  Tindak lanjut &rarr;
                </button>
              </div>
            </div>
            ...
            <div class="rounded-xl p-4 bg-[#F7F9F4] border-l-4 border-l-[#4A5B3A] border border-[#E5E0D8]/60 shadow-xs">
              ...
              <div class="flex justify-end pt-2">
                <button class="px-3 py-1 rounded-lg bg-white text-[#4A5B3A] border border-[#E5E0D8] text-xs font-bold hover:bg-[#F7F9F4] transition-colors shadow-2xs">
                  Tindak lanjut &rarr;
                </button>
              </div>
            </div>
            ...
            <div class="rounded-xl p-4 bg-[#FFFDF5] border-l-4 border-l-[#C28E3A] border border-[#E5E0D8]/60 shadow-xs">
              ...
              <div class="flex justify-end pt-2">
                <button class="px-3 py-1 rounded-lg bg-white text-[#C28E3A] border border-[#E5E0D8] text-xs font-bold hover:bg-[#FFFDF5] transition-colors shadow-2xs">
                  Tindak lanjut &rarr;
                </button>
              </div>
            </div>
          ...
          <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-[#E5E0D8]/60 text-xs font-medium text-[#231a10]">
              <tr v-for="(log, idx) in activityLogs" :key="'log-' + idx" class="hover:bg-[#FFF8F4] transition-colors">
                <td class="py-3.5 px-4 font-mono text-[12px] text-[#7E7063] whitespace-nowrap">{{ log.time }}</td>
                <td class="py-3.5 px-4 font-semibold text-[#231a10]">{{ log.title }}</td>
                <td class="py-3.5 px-4 text-[#645d58]">{{ log.operator }}</td>
                <td class="py-3.5 px-4 text-right">
                  <span class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-[11px] font-bold" :class="log.statusClass">
